...
class SQtree {
 ...
 private:
  ...
  pair <int, int> findBest (stats & s, Node * toSplit, int x, int y);
  void renderTree(Node* treePrev, PNG & picToDraw);
  void clearSub(Node* parentNode);
  int countChild(Node* toCount);
  Node * copyHelp (Node * thisCur); 
